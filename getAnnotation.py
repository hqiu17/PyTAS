...
def get_query_data(qryfile, data, keep_all):
    ...
    Arguments:
        qryfile: a file containing list of queries with security names at the first 
            column. If more than one columns present, the file should be tab delimited.
        data: a pandas dataframe containing information to be retrieved. The index is a
            list of security name
        keep_all: a boolean. If true, query securities not present in 'data' will not be
            discarded. Instead, their attributes are filled with '0'
             
    Return:
        a dataframe with rows representing query securites
    """
    ...
    hit = 0
    for line in queries:
        symbol=line.rstrip().split("\t")[0]     # security name is at the 1st column
        if symbol == "Symbol":
            continue
...
if __name__ == "__main__":
    ...
    if os.path.exists(data_file):
        try:
            ...
            print (f"Pandas error with openning {data_file}, ", sys.exc_info()[0])
            raise
